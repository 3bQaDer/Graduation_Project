...
void ESP_voidInit(void)
{
	...
	while(Local_u8Result == 0)
	{
		MUSART1_vidSendTextSynch("AT+CWJAP_CUR=\"WE_B3B3D4\",\"M0501047410\"\r\n");
		Local_u8Result = Esp_u8ValidateCmd();
	}
	while(Local_u8Result == 0)
	{
		MUSART1_vidSendTextSynch("AT+CIPSEND=44\r\n");
		MUSART1_vidSendTextSynch("GET http://yoriichi.freevar.com/status.txt");
		Local_u8Result = Esp_u8ValidateCmd();
	}
}
